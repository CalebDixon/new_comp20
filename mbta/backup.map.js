...
function getSchedule(ID)
{
	...
	xmlHttp.open( "GET","https://defense-in-derpth.herokuapp.com/redline/schedule.json?stop_id=" + ID, false );
	xmlHttp.send( null );
...
function initMap() {
	...
	var close_station_name = 'temp_station';
	var close_station_distance = 10000000000;
	...
	for (i = 0; i < 22; i++){
		...
		if(distance < close_station_distance){
			close_station_distance = distance;
			close_station_name = stopsJSON[i].stop_name;
			close_station_coord = stopsJSON[i].latlng;
		}
	}	
	 ...
	 var contentString = '<div id="content">'+
        	    '<div id="siteNotice">'+
        	    '</div>'+
        	    '<h1 id="firstHeading" class="firstHeading">Closest Station</h1>'+
        	    '<div id="bodyContent">'+
		    'you are nearest to '+ close_station_name + ' station' +
		    '<p></p>' +
		    'this station is ' + close_station_distance + ' miles away' +
        	    '</div>'+
        	    '</div>';
